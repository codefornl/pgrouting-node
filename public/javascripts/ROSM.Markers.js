...
ROSM.extend(ROSM.Markers, {
  ...
  setSource: function(position) {
    if (this.route[0] && this.route[0].label == ROSM.CONSTANTS.SOURCE_LABEL) {
      ...
      this.route.splice(0, 0, new ROSM.Marker(ROSM.CONSTANTS.SOURCE_LABEL, {
        icon: this.beginIcon
      }, position));
    }
  ...
  setTarget: function(position) {
    if (this.route[this.route.length - 1] && this.route[this.route.length - 1].label == ROSM.CONSTANTS.TARGET_LABEL) {
      ...
      this.route.splice(this.route.length, 0, new ROSM.Marker(ROSM.CONSTANTS.TARGET_LABEL, {
        icon: this.endIcon
      }, position));
    }
    ROSM.G.markers.addPopup();
  ...
  addVia: function(position, desc) {
    ...
    this.route.splice(this.route.length - 1, 0, new ROSM.Marker(ROSM.CONSTANTS.VIA_LABEL, {
      icon: this.viaIcon
    }, position));
